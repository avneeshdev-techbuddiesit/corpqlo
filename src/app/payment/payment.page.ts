...
import { AlertController, NavController, Events, Platform, NavParams, ModalController } from '@ionic/angular';
...
import { OrderSucessPage } from '../order-sucess/order-sucess.page';
...
export class PaymentPage implements OnInit {
  ...
  @Input() PData_order = { apikey: this.rest.APIKey, user_id: '', store_id: '', promocode:'', promocode_data: [], express_delivery_status:0, express_delivery_price: 0, check_wallet: 0,  wallet_pin: null, timeslot_time:'', pickup_timeslot:'', paymethod: 'PAYTM', comments: '', login_by: this.rest._platform, order_type: 0, shipping_charge: 0 ,mb_express:''};
    ...
    ngOnInit() {
        this.NewTimeSlot = this.activatedRoute.snapshot.paramMap.get('pick_timeslot');
        console.log('timeslot get', this.NewTimeSlot)
        this.PData_order.timeslot_time = this.NewTimeSlot
    }
    ...
    get_checkout_data() {
        ...
        this.rest.present();
        this.rest.GlobalPHit(this.PData, 'v2/Checkout').subscribe((result) => {
            console.log(result)
            if (result.status == 1) {
                this.rest.dismiss();
    ...
    place_order() {
        ...
        // }
            ...
            this.rest.GlobalPHit(this.PData_order, 'Place_order').subscribe((result) => {
                if (result.status == 1) {
                    this.rest.dismiss();
                    // this.rest.showAlert(result.message)
                    if (result.status == 1) {
                        ...
                        if(result.data.PAYMENT_TYPE == 'COD')
                        {
                            ...
                            this.orderStatus(result.status, this.paymoney_amount, this.order_id, this.order_id)
    ...
    orderStatus(order_status, total_amount, order_id, order_number) {
        this.modalCtrl.create({
            component: OrderSucessPage,
            componentProps: { order_status: order_status, total_amount: total_amount, order_id: order_id, order_number: order_number }
        }).then((res) => {
            res.present();
        })
    }
